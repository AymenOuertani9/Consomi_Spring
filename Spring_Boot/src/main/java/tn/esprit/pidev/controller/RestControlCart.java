...
public class RestControlCart {
 ...
 // URL : http://localhost:8081/SpringMVC/servlet/getCartTotalById/{idlc}/{idcart}
    ...
    public void deleteCartById(@PathVariable("idcart")int cartId) {
  	...
  	//affecterCartAUser/1/1
  	 	@PutMapping(value = "/affectercartauser/{iduser}/{idcart}") 
  	public String affecterCartAUser(@PathVariable("iduser")int userId,@PathVariable("idcart") int cartId) {
  		return cartsev.affecterCartAUser(userId, cartId);
  	}
  	// http://localhost:8081/SpringMVC/servlet/ajouteretaffecterUserACart/1
  	  	@PostMapping("/ajouteretaffecterCartaUser/{iduser}")
  	  public String ajouteretaffecterUserACart(@RequestBody Cart cart,@PathVariable("iduser") int userId) {
  	  		return cartsev.ajouteretaffecterUserACart(cart, userId);
  	  	}

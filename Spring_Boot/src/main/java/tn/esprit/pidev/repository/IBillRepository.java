...
public interface IBillRepository extends JpaRepository<Bill, Integer>{
	...
	@Query("select coalesce(sum(p.totalfinal),0) from Bill p ")
	public Double prixAchatsProduits();
	@Query("select coalesce(sum(p.totalfinal),0) from Bill p where  p.datebill between :d1 and :d2")
	...
	public long count();
	@Query("SELECT SUM(c.price) FROM Command c "
			+ "join c.cart ca "
			+ "join ca.user u "
			+ "where u.iduser=:userId")
	...
	@Query("select HOUR(s.datebill) , coalesce(avg(s.totalfinal),0) from Bill s where s.delivery.commande.idcommand=:p and YEAR(s.datebill)=:a and MONTH(s.datebill)=:m and DAY(s.datebill)=:j group by HOUR(s.datebill)")
	...
	@Query("select DAY(s.datebill) , coalesce(avg(s.totalfinal),0) from Bill s where s.delivery.commande.idcommand=:p and YEAR(s.datebill)=:a and MONTH(s.datebill)=:m group by DAY(s.datebill)")
	public List<Object[]> getMoyenBillMonth(@Param("p") int idcommand, @Param("a")Integer a, @Param("m")Integer m); 
	//la moyenne de facture par mois
	@Query("select MONTH(s.datebill) , coalesce(avg(s.totalfinal),0) from Bill s where s.delivery.commande.idcommand=:p and YEAR(s.datebill)=:a group by MONTH(s.datebill)")
